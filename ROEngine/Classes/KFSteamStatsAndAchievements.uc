...
// Siren's Belch Achievements
const KFACHIEVEMENT_WinSirensBelchNormal                = 252;
const KFACHIEVEMENT_WinSirensBelchHard                  = 253;
const KFACHIEVEMENT_WinSirensBelchSuicidal              = 254;
const KFACHIEVEMENT_WinSirensBelchHell                  = 255;
const KFACHIEVEMENT_DestroyBeerSteins                   = 256;

// Stronghold Achievements
const KFACHIEVEMENT_WinStrongholdNormal                 = 257;
const KFACHIEVEMENT_WinStrongholdHard                   = 258;
const KFACHIEVEMENT_WinStrongholdSuicidal               = 259;
const KFACHIEVEMENT_WinStrongholdHell                   = 260;
const KFACHIEVEMENT_DestroyGoldBags                     = 261;

// Transit Achievements
const KFACHIEVEMENT_WinTransitNormal                    = 262;
const KFACHIEVEMENT_WinTransitHard                      = 263;
const KFACHIEVEMENT_WinTransitSuicidal                  = 264;
const KFACHIEVEMENT_WinTransitHell                      = 265;
const KFACHIEVEMENT_WinTransitObjNormal                 = 266;
const KFACHIEVEMENT_WinTransitObjHard                   = 267;
const KFACHIEVEMENT_WinTransitObjSuicidal               = 268;
const KFACHIEVEMENT_WinTransitObjHell                   = 269;
const KFACHIEVEMENT_NitroInXSeconds                     = 270;
const KFACHIEVEMENT_KillXZedsDuringWeld                 = 271;
const KFACHIEVEMENT_PickUpDoorKey                       = 272;
...
var name TransitDNAVialsEventName;
var name TransitPickUpKeyEventName;
var name TransitKillXZedsDuringWeldEventName;
...
function WonGame(string MapName, float Difficulty, bool bLong)
{
    ...
    else if ( MapName ~= "KF-SirensBelch" )
    {
        CheckEndGameAchievements(Difficulty, KFACHIEVEMENT_WinSirensBelchNormal);
    }
    else if ( MapName ~= "KF-Stronghold" )
    {
        CheckEndGameAchievements(Difficulty, KFACHIEVEMENT_WinStrongholdNormal);
    }
    else if ( MapName ~= "KF-Transit" )
    {
        CheckEndGameAchievements(Difficulty, KFACHIEVEMENT_WinTransitNormal);
    }
    else if ( MapName ~= "KFO-Transit" )
    {
...
// Transit Achievements
function CheckTransitAchievementsCompleted()
{
	local int i, FirstTransitAchievement, LastTransitAchievement, TotalTransitAchievements;

    // skip if they already have it
    if( Achievements[KFACHIEVEMENT_CompleteMsClamleyAchievements].bCompleted == 1 )
    {
        return;
    }

    // skip if they haven't won on at least hard
    if( !HasAchievementInRangeCompleted(KFACHIEVEMENT_WinTransitObjHard, KFACHIEVEMENT_WinTransitObjHell) )
    {
    	return;
    }

    // has transit obj achievement
    TotalTransitAchievements = 1;

    FirstTransitAchievement = KFACHIEVEMENT_NitroInXSeconds;
    LastTransitAchievement = KFACHIEVEMENT_DestroyDNAVials;

    for (i = FirstTransitAchievement; i <= LastTransitAchievement; i++)
    {
        if ( Achievements[i].bCompleted == 1 )
        {
            TotalTransitAchievements++;
        }
    }

    if ( TotalTransitAchievements >= Achievements[KFACHIEVEMENT_CompleteMsClamleyAchievements].ProgressDenominator )
    {
        SetSteamAchievementCompleted( KFACHIEVEMENT_CompleteMsClamleyAchievements );
    }
}
...
simulated function CheckEvents( Name EventName )
{
    ...
    else if ( EventName == TransitDNAVialsEventName )
    {
        CheckAndSetAchievementComplete( KFACHIEVEMENT_DestroyDNAVials );
        CheckTransitAchievementsCompleted();
    }
    else if ( EventName == TransitPickUpKeyEventName )
    {
        CheckAndSetAchievementComplete( KFACHIEVEMENT_PickUpDoorKey );
        CheckTransitAchievementsCompleted();
    }
    else if ( EventName == TransitKillXZedsDuringWeldEventName )
    {
        CheckAndSetAchievementComplete( KFACHIEVEMENT_KillXZedsDuringWeld );
        CheckTransitAchievementsCompleted();
    }
    else if ( EventName == SirensBelchBeerSteinsEventName )
    {
        CheckAndSetAchievementComplete( KFACHIEVEMENT_DestroyBeerSteins );
    }
    else if ( EventName == StrongholdGoldBagsEventName )
    {
        CheckAndSetAchievementComplete( KFACHIEVEMENT_DestroyGoldBags );
    }
    ...
    else if ( EventName == TransitNitroInXSecondsFailedEventName )
    {
        bEventAchievementFailed = true;
    }
...
function OnObjectiveCompleted( name ObjectiveName )
{
    ...
    else if ( ObjectiveName == TransitNitroObjName )
    {
        CheckAndSetAchievementComplete( KFACHIEVEMENT_NitroInXSeconds );
        CheckTransitAchievementsCompleted();
    }
}
...
defaultproperties
{
     ...
     Achievements(252)=(SteamName="WinSirensBelchNormal",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_252',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(253)=(SteamName="WinSirensBelchHard",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_253',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(254)=(SteamName="WinSirensBelchSuicidal",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_254',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(255)=(SteamName="WinSirensBelchHell",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_255',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(256)=(SteamName="DestroyBeerSteins",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_274',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(257)=(SteamName="WinStrongholdNormal",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_256',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(258)=(SteamName="WinStrongholdHard",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_257',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(259)=(SteamName="WinStrongholdSuicidal",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_258',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(260)=(SteamName="WinStrongholdHell",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_259',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(261)=(SteamName="DestroyGoldBags",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_273',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(262)=(SteamName="WinTransitNormal",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_260',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(263)=(SteamName="WinTransitHard",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_261',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(264)=(SteamName="WinTransitSuicidal",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_262',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(265)=(SteamName="WinTransitHell",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_263',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(266)=(SteamName="WinTransitObjNormal",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_264',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(267)=(SteamName="WinTransitObjHard",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_265',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(268)=(SteamName="WinTransitObjSuicidal",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_266',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(269)=(SteamName="WinTransitObjHell",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_267',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(270)=(SteamName="NitroInXSeconds",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_268',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     Achievements(271)=(SteamName="KillXZedsDuringWeld",Icon=Texture'KillingFloor2HUD.Achievements.Achievement_269',LockedIcon=Texture'KillingFloorHUD.Achievements.KF_Achievement_Lock')
     ...
     TransitDNAVialsEventName="DNASamplesCompleted"
     TransitPickUpKeyEventName="GotKeycardAchievement"
     TransitKillXZedsDuringWeldEventName="KilledWeldZedsAchievement"
     ...
     SteamNameAchievement(252)="WinSirensBelchNormal"
     SteamNameAchievement(253)="WinSirensBelchHard"
     SteamNameAchievement(254)="WinSirensBelchSuicidal"
     SteamNameAchievement(255)="WinSirensBelchHell"
     SteamNameAchievement(256)="DestroyBeerSteins"
     SteamNameAchievement(257)="WinStrongholdNormal"
     SteamNameAchievement(258)="WinStrongholdHard"
     SteamNameAchievement(259)="WinStrongholdSuicidal"
     SteamNameAchievement(260)="WinStrongholdHell"
     SteamNameAchievement(261)="DestroyGoldBags"
     SteamNameAchievement(262)="WinTransitNormal"
     SteamNameAchievement(263)="WinTransitHard"
     SteamNameAchievement(264)="WinTransitSuicidal"
     SteamNameAchievement(265)="WinTransitHell"
     SteamNameAchievement(266)="WinTransitObjNormal"
     SteamNameAchievement(267)="WinTransitObjHard"
     SteamNameAchievement(268)="WinTransitObjSuicidal"
     SteamNameAchievement(269)="WinTransitObjHell"
     SteamNameAchievement(270)="NitroInXSeconds"
     SteamNameAchievement(271)="KillXZedsDuringWeld"
